...
- Data gets imported from a source, and is split between English and french
- Only the sample that have a model assigned to them and confirmed tags are kept
...
- Now that the sample and the classes are numbers, a simple classification algorithm can be retrained
...
- The model, the vectorizer data and the possible tag for each model is saved in a pickle file.
...
- The script in this repo works locally (localhost), but can be used to generate prediction on a live server
